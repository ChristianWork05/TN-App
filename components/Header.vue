    ...
    <header class="absolute inset-x-0 top-0 z-50">
      <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div class="hidden lg:flex lg:flex-1 lg:justify-end items-center space-x-4">
          <a v-if="!isLoggedIn" href="#" class="text-sm/6 font-semibold text-white" @click="showLogin = true">Ingresar <span aria-hidden="true">&rarr;</span></a>
          <a v-else href="#" class="hover:underline flex items-center text-white" @click="handleLogout">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 m-1">
              <path stroke-linecap="round" stroke-linejoin="round" d="M15 12H3m0 0l3.75-3.75M3 12l3.75 3.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Cerrar Sesión
          </a>
      ...
      <div v-if="showCart" class="fixed inset-0 z-50 flex items-center justify-end bg-black bg-opacity-50" @click.self="toggleCart">
        <transition name="slide-fade">
          <div class="relative h-full w-full lg:w-2/5 bg-white border-l rounded-l shadow-lg p-4 cart-container">
      ...
      <Dialog v-if="showLogin" @close="showLogin = false" :open="showLogin" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
          <DialogPanel class="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <button @click="showLogin = false" class="absolute top-2 right-2 text-gray-700">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
              </svg>
            </button>
        ...
        <DialogPanel class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          ...
          <div class="mt-6 flow-root">
            <div class="-my-6 divide-y divide-gray-500/10">
              ...
              <div class="py-6">
                <a v-if="!isLoggedIn" href="#" class="-mx-3 block rounded-lg py-2.5 px-3 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-400/10" @click="showLogin = true">Ingresar</a>
                <a v-else href="#" class="-mx-3 block rounded-lg py-2.5 px-3 text-base font-semibold leading-7 text-gray-900 hover:bg-gray-400/10" @click="handleLogout">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 m-1">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12H3m0 0l3.75-3.75M3 12l3.75 3.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  Cerrar Sesión
...
const handleLogout = () => {
  // Eliminar el token o la información del usuario del almacenamiento local
...
const handleLoginSuccess = () => {
  ...
  // Ocultar el componente de inicio de sesión
